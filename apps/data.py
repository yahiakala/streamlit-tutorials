...
def app():
    ...
    Images are input into models as arrays of numbers that represent
    the pixel colour (in this case, the brightness/darkness of the pixel).
    Models, such as the convolutional neural network (CNN) in this app,
    then apply a number of 'kernels' to them to decipher what patterns are
    in the images. This particular project focuses on 'smiles'.
    """)
    imgres = st.slider('Pick the resolution (number of pixels)', 5, 28)
    ...
    if uploaded_file:
        ...
        fixed_imageb = imutils.resize(gray, width=28) # 28 x 28 x 1
